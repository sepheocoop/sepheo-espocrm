...
const promiseAllWait = (promises) => {
    ...
    return Promise.all(all_promises).then((results) => {
...
exports.moveDir = (from_dir, to_dir) =>
    fs.readdir(from_dir).then((children) =>
        fs
            .ensureDir(to_dir)
            .then(() => {
                let move_promises = [];
                ...
                return promiseAllWait(move_promises).catch((err) => {
                    let undo_move_promises = [];

                    for (
                        let i_moved_record = 0;
                    ...
                    return promiseAllWait(undo_move_promises).then(() => {
                        throw err;
                    });
                });
            })
...
exports.camelCaseToHyphen = (string) =>
