...
There are a few things which need to be set up for the local EspoCRM application to work correctly.

To do that, log in as admin (using the log-in details above), then follow these steps.

#### 1. Set the Site URL

This needs to be correct for the registration / edit links sent out to work.

- Navigate via the triple-dot drop-down in the top right of the daskboard to "Administration".
- Select "Settings - System settings of application"
- Check that the Site URL parameter is set correctly. For normal development, it should be http://localhost:8080 - i.e. make sure it includes the port, which it does not by default.
- Click "Save"

![EspoCRM Settings](assets/Screenshot_Settings.png)

#### 2. Set up outbound email

In a production deployment, you'd do this with a real SMTP account for the purpose. Details vary and somewhat out of scope here - for development you can use a mock SMTP service like [SMTP Bucket](https://www.smtpbucket.com)
...
- Set the Password parameter to a random password of your choice (it is ignored)
- Leave the other fields unchanged
- Click "Send Test Email" to check this will work

![Create Email Account - SMTP](assets/Screenshot_Create_Group_Email_Account_SMTP.png)

Click the "Save" button at the top to ensure this persists.
...
> [!NOTE]
>
> The `docker-copy.js` script adds in some stock custom fields for Contact entities such that the registration form values have somewhere to get saved in a development environment.
>
> For a production system, there may be existing custom fields set already. You need to review the fields added for development purposes and add them to your deployment, and resolve any differences / clashes which may occur. See the following sections.
